<?php
 header("Content-Type: text/html; charset=UTF-8");
  ...
  if(isset($_POST['update'])){
    $q ="UPDATE Krmi SET
            kod_zvirete=".var_export($_POST['animal'], true).",
            datum_krmeni=".var_export($_POST['date'], true).",
            od=".var_export($_POST['from'], true).",
            do=".var_export($_POST['to'],true).",
            mnozstvi_zradla=".var_export($_POST['foodAmount'],true)."
  ...
  <body>
    ...
    <div id="content">
      <!-- Input form -->
        ...
        FROM Osetrovatel as O JOIN Krmi as K JOIN Zvire as Z WHERE O.os_cislo=K.os_cislo AND
        K.kod_zvirete=Z.kod_zvirete AND O.os_cislo='".$_SESSION['os_cislo']."'";
        /*$q= "SELECT K.datum_krmeni, K.od, K.do, K.mnozstvi_zradla
        FROM Osetrovatel as O JOIN Krmi as K JOIN Vybeh as V WHERE O.os_cislo=K.os_cislo AND
        K.kod_zvirete=V.cislo_vybehu AND O.os_cislo='".$_SESSION['os_cislo']."'";*/
        $q = $q . "LIMIT 1 OFFSET ". $_GET['pk']."";
        $res=$db->query($q);
        $res = $res->fetch_assoc();
        if(!$res)
